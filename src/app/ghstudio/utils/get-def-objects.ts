import { GhXmlType, XY, XYWH } from "@/types/types";
...
function getSizeAndCordinate(compoentData: AttributeContainerType[][]) {
	...
	const sizeOfScript = getSizeOfScript(xywh);

	const totalArea = xywh.map((c) => c.w * c.h).reduce((a, b) => a + b, 0);
	const density =
		Math.round((totalArea / (sizeOfScript.x * sizeOfScript.y)) * 1000) / 1000;

	return {
		xywh: xywh,
		sizeOfScript: getSizeOfScript(xywh),
		density: density,
	};
}

function getSizeOfScript(xy: XYWH[]): XY {
	...
	const widthOfBottomRight = xy[bottomRightIndex].w;
	const heightOfBottomRight = xy[bottomRightIndex].h;

	const width = x + widthOfBottomRight;
	const height = y + heightOfBottomRight;
